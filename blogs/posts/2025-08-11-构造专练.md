...
int main() {
    ...
    for(int i = 1; i <= n; i ++) {
        ...
        for(int j = rmax[i + 1]; j <= m; j ++) r[j] = qmi((LL)j, suf[i + 1]);
    // 注意边界！！！
        for(int j = 2; j <= m; j ++) ans = (ans + l[j] * (r[j - 1] - r[j - 2]) % Mod) % Mod;
    // 由于 b[i] >= 1，所以左边的最小值至少都要是 2，右边的意思是恰好最大值是 j - 1 的情况，所以这里是在固定右边最大值
    }
    (ans += qmi(m, suf[1])) %= Mod;
    ans = (ans % Mod + Mod) % Mod;
    cout << ans << '\n';
